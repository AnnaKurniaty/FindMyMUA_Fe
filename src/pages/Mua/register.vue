            ...
            >
                ...
                <form @submit.prevent="handleSubmit" class="space-y-4">
                    <div class="md:grid md:grid-cols-2 md:gap-6 lg:grid-cols-3 md:space-y-0">
                        <div>
                            <label class="block text-[#D56E6E] font-medium mb-2">Brand Name/Makeup Artist Name</label>
                            <input
                                type="text"
                        ...
                        <div>
                            <div>
                                ...
                                <p data-id="Xus1" class="text-xs text-pink-800 mt-1">
                        ...
                        <div>
                            <div>
                                <label class="block text-[#D56E6E] font-medium mb-2"
                                ...
                                <div class="relative">
                                    ...
                                    />
                                    <span
                                        class="material-symbols-outlined absolute right-2 top-2 text-[#D56E6E] cursor-pointer hover:text-purple-600 transition-colors"
                                        >visibility_off</span
                                    >
                                </div>
                                <p class="text-xs text-pink-800 mt-1">Both passwords must match</p>
                            </div>
                        </div>
                        <div>
                        ...
                        <div>
                            <div class="flex items-center justify-between">
                                ...
                                <span class="text-xs text-pink-500 cursor-pointer hover:text-pink-600 transition-colors flex items-center">
                                    <button type="button" @click="getLocation" class="material-symbols-outlined text-sm mr-1">my_location</button> Use
                                    My Current Location
                                </span>
                            </div>
                            <div class="grid grid-cols-2 gap-4">
                                <input
                                    ...
                                    v-model="form.studio_lat" readonly
                                />
                                <input
                                    type="text"
                                    class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:outline-none focus:ring-2 focus:ring-pink-300 focus:border-transparent transition-all"
                                    placeholder="Longitude"
                                    v-model="form.studio_lng" readonly
                                />
                            </div>
                        </div>
                        <div>
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <div class="flex flex-wrap gap-2">
                                        ...
                                        >
                                            <input type="checkbox" :value="day" v-model="form.available_days" class="w-3 h-3 text-pink-800 rounded mr-1 focus:ring-[#D56E6E]" />
                                ...
                                <div>
                                    ...
                                    <input
                                        class="w-full p-2 border-2 border-pink-200 rounded-lg focus:border-[#D56E6E] focus:outline-none transition-colors bg-pink-50 mb-2"
                                         type="time" v-model="form.available_start_time"
                                    />
                                </div>
                                <div>
                                    ...
                                    <input
                                        class="w-full p-2 border-2 border-pink-200 rounded-lg focus:border-[#D56E6E] focus:outline-none transition-colors bg-pink-50 mb-2"
                                         type="time" v-model="form.available_end_time"
                                    />
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="space-y-3">
                            <div v-for="(cert, index) in form.certification" :key="index" class="flex space-x-2 items-center">
                                <input
                                v-model="form.certification[index]"
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <label class="flex items-center space-x-2 cursor-pointer" v-for="sp in specializations" :key="sp">
                                    <input
                                        type="checkbox"
                                        class="w-4 h-4 text-pink-800 rounded focus:ring-[#D56E6E]"
                        ...
                        <div>
                            <label class="block text-[#D56E6E] font-medium mb-2"
                                >Color/Skin/Technique Preferences</label
                            >
                            <div class="space-y-3">
                                <div>
                                ...
                                <div>
                                    <label class="text-xs text-pink-800 mb-1 block"
                                        >Skin Types You Specialize In</label
                                    >
                                    <div class="flex flex-wrap gap-2">
                                        <label
                                            class="inline-flex items-center p-2 rounded-lg hover:bg-pink-100 cursor-pointer transition-colors"
                ...
                <div class="mt-6 text-center">
                    <p class="text-pink-800 text-sm">
                        ...
                        <a href="#" class="text-[#D56E6E] font-medium hover:text-purple-700 ml-1">Sign in here</a>
                    </p>
                </div>
            </div>
        </div>
...
async function handleSubmit() {
  ...
  try {
    const regRes = await fetch('http://localhost:8000/api/auth/register/mua', {
      method: 'POST',
    ...
    const user_id = regData.user?.id

    const fd = new FormData()
    fd.append('bio', form.bio)
    fd.append('certification', JSON.stringify(form.certification))
    fd.append('service_area', form.service_area)
    fd.append('studio_lat', form.studio_lat)
    fd.append('studio_lng', form.studio_lng)
    fd.append('available_start_time', form.available_start_time + ':00')
    fd.append('available_end_time', form.available_end_time + ':00')
    fd.append('available_days', JSON.stringify(form.available_days))
    fd.append('makeup_specializations', JSON.stringify(form.makeup_specializations))
    fd.append('makeup_styles', JSON.stringify(form.makeup_styles))
    fd.append('skin_type', JSON.stringify(form.skin_type))
    if (form.profile_photo) fd.append('profile_photo', form.profile_photo)

    const profileRes = await fetch('http://localhost:8000/api/mua/profile', {
      method: 'POST',
      headers: {
    ...
    if (!profileRes.ok) return alert(profileData.message || 'Profile creation failed')

    alert('MUA Account Created!')
    router.push('/login')
  } catch (err) {
    console.error(err)
